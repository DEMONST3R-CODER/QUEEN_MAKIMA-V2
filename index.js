...
async function startA17() {
  console.log(
    color(
      figlet.textSync("QUEEN MAKIMA V2", {
        font: "Standard",
  ...
  console.log(color('\nTHANKS FOR CHOOSING THUGNIFICENT TECH™\n\nTHANKS FOR USER : QUEEN MAKIMA V2', 'aqua'))
  console.log(color('\nKINDLY TYPE . support TO FOLLOW OUR CHANNEL FOR UPDATES', 'aqua'))

  const { state, saveCreds } = await useMultiFileAuthState("./SESSION");
  const A17 = A17Connect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["QUEEN MAKIMA_V2", "Safari", "3.O"],
    auth: state,
  });
